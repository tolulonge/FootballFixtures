            ...
            with(todayFixture){
                ...
                when(status){
                    MatchStatus.IN_PLAY ->{
                        ...
                        binding.imgLiveIndicator.setBackgroundColor(ContextCompat.getColor(binding.root.context, R.color.green))

                    }
                    MatchStatus.PAUSED ->{
                        binding.imgLiveIndicator.setBackgroundColor(ContextCompat.getColor(binding.root.context, R.color.yellow))
                        binding.txtFullTimeIndicator.text = "PS"
                    }
                    MatchStatus.FINISHED -> {
                        binding.txtFullTimeIndicator.text = "FT"
                        binding.imgLiveIndicator.setBackgroundColor(ContextCompat.getColor(binding.root.context, R.color.brown))
                    }
                    MatchStatus.TIMED -> {
                        binding.txtFullTimeIndicator.text = "NS"
                        binding.imgLiveIndicator.setBackgroundColor(ContextCompat.getColor(binding.root.context, R.color.black))
                    }
