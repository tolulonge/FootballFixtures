...
class CompetitionFixturesViewModel @Inject constructor(
    ...
    private val _fixtures =
        MutableStateFlow<CompetitionFixturesFragmentUiState>(CompetitionFixturesFragmentUiState.Empty)
    ...
    fun onEvent(event: FootballFixturesEvent) {
        when (event) {
            is FootballFixturesEvent.GetCompetitionFixtures -> {
                getMatchDayCompetitionFixtures(
     ...
     */
    private fun getMatchDayCompetitionFixtures(
        fetchFromRemote: Boolean,
        competitionCode: String,
        matchDay: Int
    ) {
        viewModelScope.launch {
            footballFixturesUseCases.getCompetitionFixturesList(
                fetchFromRemote,
                competitionCode,
                matchDay
            )
                .collect { result ->
                    when (result) {
                        is Resource.Success -> {
                            result.data?.let { fixtures ->
                                ...
                                _fixtures.value = CompetitionFixturesFragmentUiState.Loaded(
                                    allPresentationMappers.domainCompetitionFixtureToPresentationCompetitionFixtureMapper.map(
                                        fixtures
                                    ),
                                    result.message ?: ""
                                )
                        ...
                        is Resource.Error -> {
                            _fixtures.value =
                                CompetitionFixturesFragmentUiState.Error(result.message ?: "")
                            _fixtures.value = CompetitionFixturesFragmentUiState.Loading(false)

                        }
                        is Resource.Loading -> {
                            _fixtures.value =
                                CompetitionFixturesFragmentUiState.Loading(result.isLoading)
                        }
                    }
                }
        }
